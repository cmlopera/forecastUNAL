#' Función `pruebaDW1()`
#'
#' Realiza test Durbin-Watson para autocorrelacion de orden 1 en un MRLM. Usa la función `durbinWatsonTest()` de la librería `car`.
#'
...
#' * `rho(1) estimado` estimación de la autocorrelacion de orden 1 `rho(1)` usando los residuales del modelo.
#' * `Estadistico D-W` El valor del estadistico de la prueba.
#' * `VP H1: rho(1)>0` El valor P para el test `H0: rho(1)=0`, vs. `H1:rho(1)>0`.
#' * `VP H1: rho(1)<0` El valor P para el test `H0: rho(1)=0`, vs. `H1:rho(1)<0`.
#' @export
#' @examples
#' #Simulando serie con modelo de tendencia lineal y errores iid normales
#' t <- 1:300
#' yt <- 3500+200*t+rnorm(300,sd=4000)
#' yt <- ts(yt,freq=12,start=c(1980,1))
#' #Gráfico de la serie simulada
#' plot(yt)
#' #Ajuste del modelo de tendencia lineal y errores iid normales
#' mrl <- lm(yt~t)
#' summary(mrl)
