...
#' @return La función no genera ningún tipo de objeto R sobre el cual luego pueda obtenerse algún valor o resultado, solo las gráficas pedidas y pruebas en consola R.
#' @export
#' @examples
#' library(forecast)
#' library(lmtest)
...
diagresTSModel <- function(modelo,RMSE,all=FALSE,single=TRUE,lagmax=36){
  ...
  res2 <- acflimits(stats::acf(as.numeric(stats::residuals(modelo)),lag.max=lagmax,plot=F))
  ...
  if(min(res2$lag)>0){
    ...
    g3<-ggplot2::ggplot(dataACF,ggplot2::aes(x=lag,xend=lag,y=0,yend=acf))+
      ...
      ggplot2::geom_segment()+
      ggplot2::geom_line(ggplot2::aes(x=lag,y=clim,color=4),linetype = 2)+
      ggplot2::geom_line(aes(x=lag,y=-clim,color=4),linetype = 2)+
      ggplot2::geom_hline(yintercept=0,color='black')+
      ggplot2::theme(legend.position = "none")+
      ggplot2::ylab("ACF")+
      ggplot2::ggtitle("ACF residuos")+
