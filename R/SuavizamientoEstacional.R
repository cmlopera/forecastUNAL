...
#' @param h Número de pronósticos que se realizarán inmediatamente despues del ajuste. Por defecto es 1.
...
#' @return La funcion produce un objeto tipo lista con los siguientes componentes:
#' * `coefficients` Un `data.frame` con una sola columna de longitud `s+2`, que con los valores finales suavizados del nivel, la pendiente y los efectos estacionales (estos ultimos en el orden `i=1, 2, ...,s`).
...
#' * `forecast` Objeto serie de tiempo multivariada (`mts`), con los pronósticos puntuales (columna 1), y limites de prediccion del 95% de confianza (columnas 2 y 3), por defecto, para `h=1` periodos después del ajuste.
